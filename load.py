...
def calculate_STH_Z_1(l, f, h, n,xh, xo, Eg1,Eg2, VLC):
    PH = []
    STH1 = []
    C = []
    ...
    for pH in range(0, 15):
        ...
        if Xh9 >= 0 and Xo9 >= 0:
            PH.append(pH)
            E = get_E_z(Xh9, Xo9, Eg1,Eg2)
            Eg = max(Eg1,Eg2)
            Egg, Eh, Ef = calculate_STH(l, f, h, n, Eg, E, VLC)
            nabs = Egg / 1000.37
            ncu = Eh * 1.23 / Egg
            STH = nabs * ncu/2
            correctedSTH = STH * 1000.37 / (1000.37 + VLC * Ef)
...
def get_E_z(j, i, E1, E2):
    ...
    if j >= 0.2 and i >= 0.6:
        ...
        E = max(E1 + 0.2 - j, E2 + 0.6 - i)
    return E
# General 类型Conversion and Janus material
class Conventional_photocatalysts:
...
# Heterojunction_Z 类型
class Z_scheme_systems:
    def __init__(self, file_path):
        self.l, self.f, self.h, self.n = load_data(file_path)

    def calculate_1(self, Eg1, Eg2):
        E1 = max(Eg1, Eg2)
        if E1 < 0.615:
            E1 = 0.615
        STH = calculate_STH_Z(self.l, self.f, self.h, self.n, E1)
        return STH
    def calculate_2(self, Eg1, Eg2,xh,xo):
        E1 = max(Eg1, Eg2)
        if E1 < 0.615:
            E1 = 0.615
        STH = calculate_STH_Z_1(self.l, self.f, self.h, self.n, xh, xo, Eg1,Eg2, 0)
        return STH
...
# Janus_Z 类型
class Janus_Z_scheme_heterojunctions:
    def __init__(self, file_path):
        self.l, self.f, self.h, self.n = load_data(file_path)

    def calculate_max(self, xh, xo, Eg, VLC):
        title_1 = ["pH", '\u03c7(H2) (eV)', '\u03c7(O2) (eV)', "\u03B7abs (%)", "\u03B7cu (%)", "\u03B7STH (%)",
                   '\u03B7\u2032STH (%)']
        PH = []
        STH1 = []
    ...
    def calculate_min(self, xh, xo, Eg, VLC):
        title_1 = ["pH", '\u03c7(H2) (eV)', '\u03c7(O2) (eV)', "\u03B7abs (%)", "\u03B7cu (%)", "\u03B7STH (%)",
                   '\u03B7\u2032STH (%)']
        PH = []
        STH1 = []
